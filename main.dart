...
class FirstScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    HBlock = SizeConfig.safeBlockHorizontal;//
    VBlock = SizeConfig.safeBlockVertical;
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
   ...
   //   ),
          ...
          // height: 800,
             child:
             CustomPaint(painter:OpenPainter(100,100,10)),
         ),
      ]),
    );
  }
...
//double height = 150.0;
  ...
  Widget build(BuildContext context) {
    ...
    final screenUnits = screenWidth/10.0;
...
class _customMultiChildLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(5)) {
      ...
      CustomPaint(painter:OpenPainter(xHeight,xWidth,xUnits));
...
Container _stateChgCont(int buttonState) {
...
//constructor
  ...
  void paint(Canvas canvas, Size size) {
    ...
    paintText(
        Offset(0.8 * sUnits, 9.0 * sUnits), canvas, size, 10, Colors.black,
        'Area of triangle = Opp * Adj / 2');
    paintText(
        Offset(0.8 * sUnits, 10.0 * sUnits), canvas, size, 10, Colors.black,
        'Area of 4 congruent triangles = 2 * Opp * Adj');

    paintText(
        Offset(0.8 * sUnits, 11.0 * sUnits), canvas, size, 10, Colors.black,
        'Area of  Big square = ( Opp + Adj ) ^2');
    paintText(
        Offset(0.8 * sUnits, 12.0 * sUnits), canvas, size, 10, Colors.black,
        ' = Opp ^ 2 + 2 * Opp * Adj + Adj ^ 2');

    paintText(
        Offset(0.8 * sUnits, 12.6 * sUnits), canvas, size, 10, Colors.black,
        'Area small square  ');
    paintText(
        Offset(0.8 * sUnits, 13.0 * sUnits), canvas, size, 10, Colors.black,
        '= Area of big square - area of the 4 triangles');
    paintText(
        Offset(0.8 * sUnits, 13.4 * sUnits), canvas, size, 10, Colors.black,
        '= Opp ^ 2 + 2 * Opp * Adj + Adj ^ 2 - 2 * Opp * Adj ');
    paintText(
        Offset(0.7 * sUnits, 13.8 * sUnits), canvas, size, 10, Colors.black,
        ' = Opp ^ 2 + Adj ^ 2 = Hyp^2');
    // paintText(Offset(0.7*sUnits,14.7*sUnits),canvas,size,10,Colors.black, '= Opp ^ 2 + 2 * Opp * Adj + Adj ^ 2 - 2 * Opp * Adj = Opp ^ 2 + Adj ^ 2');
    // paintText(Offset(0.7*sUnits,15.1*sUnits),canvas,size,10,Colors.red, '   Opp ^ 2 + Adj ^ 2 = Hyp ^ 2');
    //  paintText(Offset(3.4*sUnits,9.62*sUnits),canvas,size,20,Colors.black, '*');
    //  paintText(Offset(3.6*sUnits,9.6*sUnits),canvas,size,18,Colors.black, 'area of the triangle');

    /* paintText(
        Offset(0.1 * sUnits, 8.8 * sUnits), canvas, size, 18, Colors.black,
        'Area of triangle = ');
    paintText(Offset(1.9 * sUnits, 8.8 * sUnits), canvas, size, 18, Colors.red,
        'Opp');
    paintText(
        Offset(2.4 * sUnits, 8.84 * sUnits), canvas, size, 20, Colors.black,
        '*');
    paintText(
        Offset(2.6 * sUnits, 8.80 * sUnits), canvas, size, 18, Colors.green,
        'Adj');
    paintText(
        Offset(3.0 * sUnits, 8.80 * sUnits), canvas, size, 18, Colors.black,
        '/ 2');

    paintText(
        Offset(0.1 * sUnits, 9.6 * sUnits), canvas, size, 18, Colors.black,
        'Area of 4 congruent triangles = 4');
    paintText(
        Offset(3.4 * sUnits, 9.62 * sUnits), canvas, size, 20, Colors.black,
        '*');
    paintText(
        Offset(3.6 * sUnits, 9.6 * sUnits), canvas, size, 18, Colors.black,
